...
class STTNInpaint:
    ...
    def __call__(self, input_frames: List[np.ndarray], input_mask: np.ndarray):
        ...
        # 初始化帧存储变量
        # 高分辨率帧存储列表（避免 deepcopy，逐帧引用即可，输出时在新列表中构建）
        ...
        if inpaint_area:
            for j in range(n_frames):
                frame = frames_hr[j].copy()  # 浅拷贝一份用于输出（避免修改原帧）
                # 对于模式中的每一个段落
                for k in range(len(inpaint_area)):
    ...
    def inpaint(self, frames: List[np.ndarray]):
        ...
        优化: torch.inference_mode() + FP16 AMP + 非阻塞传输
        """
        frame_length = len(frames)
        # 对帧进行预处理转换为张量，并进行归一化
        feats = _to_tensors(frames).unsqueeze(0) * 2 - 1
        # 把特征张量转移到指定的设备（CPU或GPU），非阻塞传输
        feats = feats.to(self.device, non_blocking=True)
        # 初始化一个与视频长度相同的列表，用于存储处理完成的帧
        comp_frames = [None] * frame_length
        # ── 推理模式（比 no_grad 更快，完全禁用 autograd）──
        with torch.inference_mode():
            # FP16 混合精度：编码器推理（CNN 密集计算，FP16 加速显著）
            ...
            while _current_frame < frame_info['len']:
                ...
                for k in range(len(inpaint_area)):
                    if len(frames[k]) > 0:  # 确保有帧可以处理
                        ...
                        comps[k] = []
                # ── 批间清理 GPU 显存碎片 ──
                if self.sttn_inpaint.device.type == 'cuda':
