...
class VideoDisplayComponent(QWidget):
    ...
    watermark_captured = Signal(object)  # 水印模板截取完成，传递 BGR numpy 数组
    ...
    def __init__(self, parent=None):
        ...
        # 水印模板截取模式
        self.watermark_capture_mode = False
        self.watermark_capture_rect = None  # 截取时的临时矩形 (ratio coords)
        self._current_frame_bgr = None  # 当前显示的原始帧（用于截取）
    ...
    def update_video_display(self, frame, draw_selection=True):
        ...
        # 保存原始 BGR 帧副本（用于水印截取）
        self._current_frame_bgr = frame.copy()
    ...
    def update_preview_with_rect(self, rect=None, draw_selection=True):
        ...
        # ---- 绘制水印截取模式框（蓝色虚线） ----
        if self.watermark_capture_mode:
            capture_rect = self.watermark_capture_rect
            if self.is_drawing:
                capture_rect = self.watermark_capture_rect
            # 使用 selection_rect fallback（如果在截取模式下 is_drawing 用的也是 selection_rect）
            if self.is_drawing and self.watermark_capture_rect is not None:
                display_size = self.video_display.size()
                pixmap_size_2 = self.current_pixmap.size()
                sx = pixmap_size_2.width() / display_size.width() if display_size.width() > 0 else 1
                sy = pixmap_size_2.height() / display_size.height() if display_size.height() > 0 else 1
                vdw = self.video_display.width()
                vdh = self.video_display.height()
                ymin, ymax, xmin, xmax = self.watermark_capture_rect
                pixel_rect = QRect(
                    int(xmin * sx * vdw),
                    int(ymin * sy * vdh),
                    int((xmax - xmin) * sx * vdw),
                    int((ymax - ymin) * sy * vdh),
                )
                # 蓝色虚线
                pen = QtGui.QPen(QtGui.QColor(0, 120, 255), 2, Qt.DashLine)
                painter.setPen(pen)
                painter.setBrush(QtGui.QColor(0, 120, 255, 40))  # 半透明蓝色填充
                painter.drawRect(pixel_rect)
                ...
                label_y = pixel_rect.top() - 5 if pixel_rect.top() > 20 else pixel_rect.bottom() + 20
                painter.drawText(pixel_rect.left(), label_y, "框选水印区域 → 松开鼠标确认")
    ...
    def selection_mouse_press(self, event):
        """鼠标按下事件处理"""
        if self.watermark_capture_mode:
            print(f"[VideoDisplay] MousePress called: capture_mode=True, button={event.button()}, enable_mouse={self.enable_mouse_events}")
        if not self.enable_mouse_events:
            print(f"[VideoDisplay] Mouse press BLOCKED: enable_mouse_events=False, capture_mode={self.watermark_capture_mode}")
            return
        
        # ---- 水印截取模式 ----
        if self.watermark_capture_mode and event.button() == Qt.LeftButton:
            print(f"[VideoDisplay] Capture mouse PRESS: is_drawing=True, pos={event.pos()}")
            ...
            self.watermark_capture_rect = (y_ratio, y_ratio, x_ratio, x_ratio)
    ...
    def selection_mouse_move(self, event):
        ...
        # ---- 水印截取模式：更新截取矩形 ----
        if self.watermark_capture_mode and self.is_drawing:
            video_display_width = self.video_display.width()
            video_display_height = self.video_display.height()
            pos = event.pos()
            y_ratio = max(0, min(1, (pos.y() - self.border_top) / video_display_height)) if video_display_height > 0 else 0
            ...
            ymin = min(start_y, y_ratio)
            ymax = max(start_y, y_ratio)
            xmin = min(start_x, x_ratio)
            xmax = max(start_x, x_ratio)
            self.watermark_capture_rect = (ymin, ymax, xmin, xmax)
            # 重绘以显示蓝色截取框
            self.update_preview_with_rect()
            return
    ...
    def selection_mouse_release(self, event):
        ...
        if self.watermark_capture_mode and self.is_drawing:
            self.is_drawing = False
            rect = self.watermark_capture_rect
            self.watermark_capture_rect = None

            if rect is not None and self._current_frame_bgr is not None:
                ymin, ymax, xmin, xmax = rect
                # 归一化矩形
                if ymin > ymax:
                    ymin, ymax = ymax, ymin
                ...
                if pw > 10 and ph > 10:
                    ...
                    roi = self._current_frame_bgr[y1:y2, x1:x2].copy()
                    self.watermark_captured.emit(roi)
    ...
    def enter_watermark_capture_mode(self):
        """进入水印模板截取模式"""
        self.watermark_capture_mode = True
        self.watermark_capture_rect = None
        self.is_drawing = False
        self.video_display.setMouseTracking(True)
        self.video_display.setCursor(Qt.CrossCursor)
        self.enable_mouse_events = True
        print(f"[VideoDisplay] Capture mode ON: watermark_capture_mode={self.watermark_capture_mode}, enable_mouse={self.enable_mouse_events}")

    def exit_watermark_capture_mode(self):
        """退出水印模板截取模式"""
        self.watermark_capture_mode = False
        self.watermark_capture_rect = None
        self.is_drawing = False
        ...
        self.enable_mouse_events = True
        self.update_preview_with_rect()
        print(f"[VideoDisplay] Capture mode OFF")
