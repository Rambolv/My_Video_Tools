...
- **多循环暴力扫除**：针对 AI 生成视频中快速变化/扭曲/变形的 Logo 水印，采用形变自适应遮罩 + RGB 聚类 + 多轮渐进清除
- **水印模板匹配检测**：支持模板截取、旋转/缩放匹配、特征匹配、颜色传播、强力清扫（时序差分检测）、强制区域重绘
- **E2FGVI 高质量修复**：CVPR 2022 流式视频修复，支持任意分辨率（HQ版本），需 48GB 以上显存
